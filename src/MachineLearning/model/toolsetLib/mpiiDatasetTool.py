...
class MPII(PoseDataset):
    def __init__(own, config): # initialize MPII as a class with PoseDataset as input var
    ...
    def mirror_joint_coords(own, joints, image_width): # mirror the joints coordinates
        joints[:, 1] = image_width - joints[:, 1]
        return joints
    def get_pose_segments(own): #return pose coordinate segments
